...
export default function($scope, context) {
        ...
        if((context.themeSettings.halo_sticky_add_to_cart == true) && ($sticky.length > 0)){
            ...
            $prodOptionDropdown.on('click', '.option-item', event => {
                ...
                if (!$target.hasClass('is-select')) {
                    ...
                    utils.api.productAttributes.optionChange($prodId, optionAttr, (err, response) => {
    ...
    function addToCart() {
        const $form = $('[data-cart-item-add-2]');
    ...
    function updateCartContent(modal, response) {
        const quantity = parseInt($body.find('.cartDesktop .cart-quantity').text()) + 1;
